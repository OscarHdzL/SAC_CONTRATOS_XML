...
namespace ApegoContractual.Controllers
{
    public class LoginController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        [HttpPost]
        public IActionResult SetRolSession(string strUsuario)
        {
            SesionRolUsuario sesionUsuario = JsonConvert.DeserializeObject<SesionRolUsuario>(strUsuario);

            HttpContext.Session.SetString("IdRol", sesionUsuario.ID_ROL);

            //borramos el contrato anterior si existia
            HttpContext.Session.SetString("IdContrato", "");

            return Json(HttpContext.Session.GetString("IdRol"));
        }
    }
}