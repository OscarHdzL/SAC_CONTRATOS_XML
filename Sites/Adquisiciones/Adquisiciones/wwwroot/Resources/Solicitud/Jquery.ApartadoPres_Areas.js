...
function catalogPresupuesto() {
    var id_dependencia = $('#HD_Dep_Id').val();
    var uri = $('#EndPointPresCore').val() + 'Contratos/Set/Presupuesto/' + id_dependencia;
...
$('#GuardarPS').click(function () {
    $('#json_pres_sol').val(JSON.stringify(baseline));
    var conteo_var = 0;
    for (var i = 0; i <= baseline.length - 1; i++) {
        conteo_var = conteo_var + parseInt(baseline[i].monto_por_ejercer);
    }
    $('#txt_montosolicitud').val(conteo_var);
