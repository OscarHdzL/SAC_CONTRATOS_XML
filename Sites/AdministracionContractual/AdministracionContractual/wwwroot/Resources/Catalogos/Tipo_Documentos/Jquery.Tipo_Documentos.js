...
function Get_lista_tipo_documento() {
    ...
    $.get($("#EndPointAdmon").val() + "Catalogos/Get/TDocumento/" + id_instancia, function (data, status) {
        var listado = [];
        console.log(data);
        for (var i = 0; i <= data.length - 1; i++) {
            ...
            fila.push("<button class='btn btn-primary' title='Editar' onclick=\"Editar_Tipo_Doc('" + data[i].id + "','" + data[i].tipo_documento + "');\"><i class='fa fa-edit'></i></button> " +
                "<button class='btn btn-danger' title='Eliminar' onclick=\"DeleteTipoDoc('" + data[i].id + "');\"><i class='fa fa-trash'></i></button> ");
            listado.push(fila);
        }

        var table = $('#tbl_tipo_documento').DataTable();
...
function Abrir_Modal_Tipo_Documento() {
    $('.clear_txt').val('');
    $('#Modal_Tipo_Documento').modal({ backdrop: 'static', keyboard: false });
    $('#Modal_Tipo_Documento').modal('show');
}

function Cerrar_Modal_Tipo_Documento() {
    $('#Modal_Tipo_Documento').modal('hide');
    $('.clear_txt').val('');
}

$("#Add_TipoDocumento").click(function () {
    Abrir_Modal_Tipo_Documento();
})

$("#btn_guardar_tipo_doc").click(function () {
    Add_Tipo_Documento();
})

function Add_Tipo_Documento() {

    if ($("#txt_tipo_documento_add").val() == "") {
        return ErrorSA('Error en los datos de entrada', 'Debe ingresar un tipo de documento.');
    }
    tbl_tipo_documento.p_tipo_documento = $("#txt_tipo_documento_add").val();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (objresponse[0].cod == "success") {
                function si() {
                    return Cerrar_Modal_Tipo_Documento();
                }
        ...
        processData: false,
        type: 'POST',
        url: $("#EndPointAdmon").val() + "Catalogos/Add/TDocumento"

    })
}

function Editar_Tipo_Doc(id, tipo_doc) {
    $('.clear_txt_upd').val("");
    $("#id_tipo_doc").val("");
    $('#Modal_Tipo_Documento_Upd').modal({ backdrop: 'static', keyboard: false });
    $('#Modal_Tipo_Documento_Upd').modal('show');
    $("#id_tipo_doc").val(id);
    $("#txt_tipo_documento_upd").val(tipo_doc);
}
...
function eliminar_tipo_doc(item) {
    $.ajax({
        url: $("#EndPointAdmon").val() + "Catalogos/Delete/TDocumento/" + item,
        dataType: 'text',
        cache: false,
        contentType: false,
        processData: false,
