function Cerrar_Modal_Tipo_Doc_Upd() {
    $('#Modal_Tipo_Proyecto_Upd').modal('hide');
    $('.clear_txt_upd').val('');
}

function Modificar_Tipo_Proyecto() {

    if ($("#txt_tipo_riesgo_upd").val() == "") {
        return ErrorSA('Error en los datos de entrada', 'Debe ingresar un tipo de riesgo.');
    }
    tbl_tipo_proyecto.p_id = $("#id_tipo_proyecto").val();
    tbl_tipo_proyecto.p_tipo_proyecto = $("#txt_tipo_proyecto_upd").val();
    ...
    $.ajax({
        ...
        processData: false,
        type: 'PUT',
        url: $("#EndPointAdmon").val() + "Catalogos/Update/TProyecto"

    })
}

$("#btn_upd_tipo_proyecto").click(function () {
    Modificar_Tipo_Proyecto();
})
